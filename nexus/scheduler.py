...
async def send_schedule_message(bot: Bot, chat_id: int, hour: int) -> None:
    ...
    # 이전 스케줄 메시지 버튼 제거
    prev_msg_id = await redis.get(_k(KEY_SCHEDULE_MSG_ID, chat_id))
    if prev_msg_id:
        try:
            await bot.edit_message_reply_markup(chat_id, int(prev_msg_id), reply_markup=None)
    ...
    # 본문·키보드 조립
    text, markup = await todo_service.build_schedule_content(chat_id, hour)
    ...
    await redis.set(_k(KEY_SCHEDULE_MSG_ID, chat_id), str(msg.message_id))
    logger.info(f"스케줄 메시지 발송 — {hour:02d}:00, msg_id: {msg.message_id}")
