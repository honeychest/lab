...
export const operationLock = createMutex();
...
export async function sendPending(planId) {
    ...
    for (const level of state.levels) {
        ...
        try {
            ...
            level.status = 'ACCEPTED';
            level.orderId = response.orderId === null || response.orderId === undefined
                ? null
...
async function reconcileLevels(state) {
    for (const level of state.levels) {
        ...
        try {
            ...
            level.status = 'ACCEPTED';
            level.orderId = order.orderId === null || order.orderId === undefined
                ? null
